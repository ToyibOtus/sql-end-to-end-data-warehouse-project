...
=========================================================================================================
Script Purpose: 
	This script performs a full load on the staging table [staging.archive_customers]
...
Usage: EXEC staging.load_staging_archive_customers;
...
=========================================================================================================
*/
CREATE OR ALTER PROCEDURE staging.load_staging_archive_customers @batch_id UNIQUEIDENTIFIER = NULL AS
BEGIN
	...
	@proc_name NVARCHAR(50) = 'load_staging_archive_customers',
	@load_type NVARCHAR(50) = 'Full',
	@ingest_source NVARCHAR(250) = 'C:\Users\PC\Desktop\archive (1)\customers.csv',
	@start_time DATETIME,
	@end_time DATETIME,
	@load_duration INT,
	...
	BEGIN TRY
		...
		TRUNCATE TABLE staging.archive_customers;

		-- Load into table
		SET @sql = 'BULK INSERT staging.archive_customers FROM ''' + @ingest_source + '''' + 
		'WITH (FIRSTROW = 2, FIELDTERMINATOR ='','', TABLOCK);';
		EXEC(@sql);

		SET @rows_loaded = @@ROWCOUNT;
		IF @rows_loaded IS NULL OR @rows_loaded = 0 THROW 50002, 'No rows loaded from file. Load aborted', 2;
