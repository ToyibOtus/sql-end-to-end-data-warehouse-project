...
=========================================================================================================
*/
CREATE OR ALTER PROCEDURE staging.load_staging_archive_orders AS
BEGIN
	-- Declare variables
	...
	BEGIN TRY
		...
		SET @layer = 'staging';
		SET @load_type = 'Full';
		SET @proc_name = 'load_staging_archive_orders';
		SET @table_loaded = 'archive_orders';
		SET @ingest_source = 'C:\Users\PC\Desktop\archive (1)\orders.csv';
		SET @start_time = GETDATE();

		-- Delete data from table
		SET @sql = 'TRUNCATE TABLE staging.archive_orders';
		EXEC(@sql);
		...
		SELECT @rows_loaded = COUNT(*) FROM staging.archive_orders;
