...
=========================================================================================================
Script Purpose: 
	This script performs a full load on the staging table [staging.load_staging_archive_sessions]
...
Usage: EXEC staging.load_staging_archive_sessions;
...
=========================================================================================================
*/
CREATE OR ALTER PROCEDURE staging.load_staging_archive_sessions @run_id UNIQUEIDENTIFIER = NULL AS
BEGIN
	...
	@proc_name NVARCHAR(50) = 'load_staging_archive_sessions',
	@load_type NVARCHAR(50) = 'Full',
	@ingest_source NVARCHAR(250) = 'C:\Users\PC\Desktop\archive (1)\sessions.csv',
	@start_time DATETIME,
	@end_time DATETIME,
	@load_duration INT,
	...
	BEGIN TRY
		...
		TRUNCATE TABLE staging.archive_sessions;

		-- Load into table
		SET @sql = 'BULK INSERT staging.archive_sessions FROM ''' + @ingest_source + '''' +
		...
		SELECT @rows_loaded = COUNT(*) FROM staging.archive_sessions;
