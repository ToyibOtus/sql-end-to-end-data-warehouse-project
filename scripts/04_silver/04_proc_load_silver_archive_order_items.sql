...
==========================================================================================================
Script Purpose:
	This script performs incremental load on the silver table [silver.load_silver_archive_order_items]. 
...
Usage: EXEC silver.load_silver_archive_order_items;
...
==========================================================================================================
*/
CREATE OR ALTER PROCEDURE silver.load_silver_archive_order_items AS
BEGIN
	...
	@proc_name NVARCHAR(50) = 'load_silver_archive_order_items',
	@load_type NVARCHAR(50) = 'Incremental',
	@ingest_source NVARCHAR(250) = 'bronze.archive_order_items',
	@start_time DATETIME,
	@end_time DATETIME,
	@load_duration INT,
	...
	BEGIN TRY
		...
		SELECT TOP 1 @batch_id = etl_batch_id FROM audit.etl_log_table 
		WHERE etl_layer = 'bronze' AND etl_table_loaded = 'archive_order_items'
		ORDER BY etl_start_time DESC;

		-- Throw an error if corresponding batch_id is NULL
		IF @batch_id IS NULL THROW 50001, 'silver.archive_order_items cannot load before bronze.archive_order_items. Load aborted.', 1;

		WITH new_records AS
		(
		SELECT bi.* FROM bronze.archive_order_items bi
		LEFT JOIN silver.archive_order_items si
		ON bi.dwh_row_hash = si.dwh_row_hash
		WHERE si.dwh_row_hash IS NULL
		)
		...
		IF @rows_source IS NULL OR @rows_source = 0 THROW 50002, 'No new records found in bronze.archive_order_items. Load aborted.', 2;
		...
		INSERT INTO silver.archive_order_items
		(
			order_id,
			product_id,
			unit_price_usd,
