...
==========================================================================================================
Script Purpose:
	This script performs incremental load on the silver table [silver.load_silver_archive_products]. 
...
Usage: EXEC silver.load_silver_archive_products;
...
==========================================================================================================
*/
CREATE OR ALTER PROCEDURE silver.load_silver_archive_products AS
BEGIN
	...
	@proc_name NVARCHAR(50) = 'load_silver_archive_products',
	@load_type NVARCHAR(50) = 'Incremental',
	@ingest_source NVARCHAR(250) = 'bronze.archive_products',
	@start_time DATETIME,
	@end_time DATETIME,
	@load_duration INT,
	...
	BEGIN TRY
		...
		etl_layer = 'bronze' AND etl_table_loaded = 'archive_products'
		ORDER BY etl_start_time DESC;

		-- Throw an error if corresponding run_id is NULL
		IF @run_id IS NULL THROW 50001, 'silver.archive_products cannot load before bronze.archive_products. Load aborted.', 1;
		...
		WHERE etl_layer = 'bronze' and etl_table_loaded = 'archive_products'
		ORDER BY etl_start_time DESC;

		WITH new_records AS
		(
		SELECT bp.* FROM bronze.archive_products bp
		LEFT JOIN silver.archive_products sp
		ON bp.dwh_row_hash = sp.dwh_row_hash
		...
		IF @rows_source IS NULL OR @rows_source = 0 THROW 50002, 'No new records found in bronze.archive_products. Load aborted.', 2;
		...
		INSERT INTO silver.archive_products
		(
			product_id,
			category,
			[name],
