...
BEGIN
	...
	@proc_name NVARCHAR(50) = 'load_silver_archive_events',
	@load_type NVARCHAR(50) = 'Incremental',
	@ingest_source NVARCHAR(250) = 'bronze.archive_events',
	@start_time DATETIME,
	@end_time DATETIME,
	@load_duration INT,
	...
	BEGIN TRY
		...
		WHERE etl_layer = 'bronze' AND etl_table_loaded = 'archive_events'
		ORDER BY etl_start_time DESC;

		-- Throw an error if corresponding batch_id is NULL
		IF @batch_id IS NULL THROW 50001, 'silver.archive_events cannot load before bronze.archive_events. Load aborted.', 1;

		WITH new_records AS
		(
		SELECT be.* FROM bronze.archive_events be
		LEFT JOIN silver.archive_events se
		ON be.dwh_row_hash = se.dwh_row_hash
		WHERE se.dwh_row_hash IS NULL
		)
		...
		IF @rows_source IS NULL OR @rows_source = 0 THROW 50002, 'No new records found in bronze.archive_events. Load aborted.', 2;
		...
		SELECT
			...
			COALESCE(CAST(cart_size AS INT), 0) AS cart_size,
			CASE 
				WHEN TRIM(payment) = 'cod' THEN 'cash_on_delivery'
				WHEN payment IS NULL THEN 'N/A'
				ELSE LOWER(TRIM(payment))
			END AS payment,
			COALESCE(CAST(discount_pct AS INT), 0) AS discount_pct,
		...
		INSERT INTO silver.archive_events
		(
			event_id,
			[session_id],
			[timestamp],
