...
BEGIN
	...
	BEGIN TRY
		...
		WHERE etl_layer = 'bronze' AND etl_table_loaded = 'archive_customers' 
		ORDER BY etl_start_time DESC;

		-- Throw an error if corresponding run_id is NULL
		IF @run_id IS NULL THROW 50001 , 'silver.archive_customers cannot load before bronze.archive_customers. Load aborted.', 1;
