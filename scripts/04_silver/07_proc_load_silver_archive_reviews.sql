...
==========================================================================================================
Script Purpose:
	This script performs incremental load on the silver table [silver.load_silver_archive_reviews]. 
...
Usage: EXEC silver.load_silver_archive_reviews;
...
==========================================================================================================
*/
CREATE OR ALTER PROCEDURE silver.load_silver_archive_reviews AS
BEGIN
	...
	@proc_name NVARCHAR(50) = 'load_silver_archive_reviews',
	@load_type NVARCHAR(50) = 'Incremental',
	@ingest_source NVARCHAR(250) = 'bronze.archive_reviews',
	@start_time DATETIME,
	@end_time DATETIME,
	@load_duration INT,
	...
	BEGIN TRY
		...
		SELECT TOP 1 @run_id = etl_run_id FROM audit.etl_log_table WHERE
		etl_layer = 'bronze' AND etl_table_loaded = 'archive_reviews'
		ORDER BY etl_start_time DESC;

		-- Throw an error if corresponding run_id is NULL
		IF @run_id IS NULL THROW 50001, 'silver.archive_reviews cannot load before bronze.archive_reviews. Load aborted.', 1;
		...
		WHERE etl_layer = 'bronze' and etl_table_loaded = 'archive_reviews'
		ORDER BY etl_start_time DESC;

		WITH new_records AS 
		(
		SELECT br.* FROM bronze.archive_reviews br
		LEFT JOIN silver.archive_reviews sr
		ON br.dwh_row_hash = sr.dwh_row_hash
		WHERE sr.dwh_row_hash IS NULL
		)
		...
		IF @rows_source IS NULL OR @rows_source = 0 THROW 50002, 'No new records found in bronze.archive_reviews. Load aborted.', 2;
		...
		SELECT
			...
			rating,
			CASE
				WHEN review_text LIKE('"%"') THEN TRIM(REPLACE(review_text, '"', ''))
		...
		INSERT INTO silver.archive_reviews
		(
			review_id,
			order_id,
			product_id,
