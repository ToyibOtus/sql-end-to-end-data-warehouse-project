...
==========================================================================================================
Script Purpose:
	This script performs incremental load on the gold table [gold.fact_reviews], and 
...
Usage: EXEC gold.load_gold_fact_reviews;
...
==========================================================================================================
*/
CREATE OR ALTER PROCEDURE gold.load_gold_fact_reviews AS
BEGIN
	...
	@table_loaded NVARCHAR(50) = 'fact_reviews',
	@proc_name NVARCHAR(50) = 'load_gold_fact_reviews',
	@load_type NVARCHAR(50) = 'Incremental',
	@ingest_source NVARCHAR(50) = 'silver.archive_reviews',
	@start_time DATETIME,
	@end_time DATETIME,
	@load_duration INT,
	...
	BEGIN TRY
		...
		WHERE etl_layer = 'silver' and etl_table_loaded = 'archive_reviews'
		ORDER BY etl_start_time DESC;

		-- Throw an error if run_id is null
		IF @run_id IS NULL THROW 50001, 'gold.fact_reviews cannot load before silver.archive_reviews. Load aborted.', 1;
		...
		WITH new_records AS
		(
		SELECT ar.* FROM silver.archive_reviews ar
		LEFT JOIN gold.fact_reviews fr
		ON ar.dwh_row_hash = fr.dwh_row_hash
		WHERE fr.dwh_row_hash IS NULL
		)
		...
		IF @rows_source IS NULL OR @rows_source = 0 THROW 50002, 'No new records found in silver.archive_reviews. Load aborted.', 2;
