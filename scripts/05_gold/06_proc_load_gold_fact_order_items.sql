...
==========================================================================================================
Script Purpose:
	This script performs incremental load on the gold table [gold.fact_order_items], and 
...
Usage: EXEC gold.load_gold_fact_order_items;
...
==========================================================================================================
*/
CREATE OR ALTER PROCEDURE gold.load_gold_fact_order_items AS
BEGIN
	...
	@table_loaded NVARCHAR(50) = 'fact_order_items',
	@proc_name NVARCHAR(50) = 'load_gold_fact_order_items',
	@load_type NVARCHAR(50) = 'Incremental',
	@ingest_source NVARCHAR(50) = 'silver.archive_order_items',
	@start_time DATETIME,
	@end_time DATETIME,
	@load_duration INT,
	...
	BEGIN TRY
		...
		WHERE etl_layer = 'silver' and etl_table_loaded = 'archive_order_items'
		ORDER BY etl_start_time DESC;

		-- Throw an error if run_id is null
		IF @run_id IS NULL THROW 50001, 'gold.fact_order_items cannot load before silver.archive_order_items. Load aborted.', 1;
		...
		WITH new_records AS
		(
		SELECT oi.* FROM silver.archive_order_items oi
		LEFT JOIN gold.fact_order_items foi
		ON oi.dwh_row_hash = foi.dwh_row_hash
		WHERE foi.dwh_row_hash IS NULL
		)
		...
		IF @rows_source IS NULL OR @rows_source = 0 THROW 50002, 'No new records found in silver.archive_order_items. Load aborted.', 2;
