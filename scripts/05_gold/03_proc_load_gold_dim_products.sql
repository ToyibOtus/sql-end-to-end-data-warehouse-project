...
==========================================================================================================
Script Purpose:
	This script performs incremental load on the gold table [gold.dim_products], and 
...
Usage: EXEC gold.load_gold_dim_products;
...
==========================================================================================================
*/
CREATE OR ALTER PROCEDURE gold.load_gold_dim_products AS
BEGIN
	...
	@table_loaded NVARCHAR(50) = 'dim_products',
	@proc_name NVARCHAR(50) = 'load_gold_dim_products',
	@load_type NVARCHAR(50) = 'Incremental',
	@ingest_source NVARCHAR(50) = 'silver.archive_products',
	@start_time DATETIME,
	@end_time DATETIME,
	@load_duration INT,
	...
	BEGIN TRY
		...
		WHERE etl_layer = 'silver' and etl_table_loaded = 'archive_products'
		ORDER BY etl_start_time DESC;

		-- Throw an error if run_id is null
		IF @run_id IS NULL THROW 50001, 'gold.dim_products cannot load before silver.archive_products. Load aborted.', 1;
		...
		WITH new_records AS
		(
		SELECT sp.* FROM silver.archive_products sp
		LEFT JOIN gold.dim_products gp
		ON sp.dwh_row_hash = gp.dwh_row_hash
		WHERE gp.dwh_row_hash IS NULL
		)
		...
		IF @rows_source IS NULL OR @rows_source = 0 THROW 50002, 'No new records found in silver.archive_products. Load aborted.', 2;
