...
==========================================================================================================
Script Purpose:
	This script performs incremental load on the gold table [gold.dim_customers], and 
...
Usage: EXEC gold.load_gold_dim_customers;
...
==========================================================================================================
*/
CREATE OR ALTER PROCEDURE gold.load_gold_dim_customers AS
BEGIN
	...
	@table_loaded NVARCHAR(50) = 'dim_customers',
	@proc_name NVARCHAR(50) = 'load_gold_dim_customers',
	@load_type NVARCHAR(50) = 'Incremental',
	@ingest_source NVARCHAR(50) = 'silver.archive_customers',
	@start_time DATETIME,
	@end_time DATETIME,
	@load_duration INT,
	...
	BEGIN TRY
		...
		WHERE etl_layer = 'silver' and etl_table_loaded = 'archive_customers'
		ORDER BY etl_start_time DESC;

		-- Throw an error if run_id is null
		IF @run_id IS NULL THROW 50001, 'gold.dim_customers cannot load before silver.archive_customers. Load aborted.', 1;
		...
		WITH new_records AS
		(
		SELECT sc.* FROM silver.archive_customers sc
		LEFT JOIN gold.dim_customers gc
		ON sc.dwh_row_hash = gc.dwh_row_hash
		WHERE gc.dwh_row_hash IS NULL
		)
		...
		IF @rows_source IS NULL OR @rows_source = 0 THROW 50002, 'No new records found in silver.archive_customers. Load aborted.', 2;
