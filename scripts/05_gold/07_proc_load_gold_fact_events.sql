...
==========================================================================================================
Script Purpose:
	This script performs incremental load on the gold table [gold.fact_events], and 
...
Usage: EXEC gold.load_gold_fact_events;
...
==========================================================================================================
*/
CREATE OR ALTER PROCEDURE gold.load_gold_fact_events AS
BEGIN
	...
	@table_loaded NVARCHAR(50) = 'fact_events',
	@proc_name NVARCHAR(50) = 'load_gold_fact_events',
	@load_type NVARCHAR(50) = 'Incremental',
	@ingest_source NVARCHAR(50) = 'silver.archive_events',
	@start_time DATETIME,
	@end_time DATETIME,
	@load_duration INT,
	...
	BEGIN TRY
		...
		WHERE etl_layer = 'silver' and etl_table_loaded = 'archive_events'
		ORDER BY etl_start_time DESC;

		-- Throw an error if run_id is null
		IF @run_id IS NULL THROW 50001, 'gold.fact_events cannot load before silver.archive_events. Load aborted.', 1;
		...
		WITH new_records AS
		(
		SELECT se.* FROM silver.archive_events se
		LEFT JOIN gold.fact_events fe
		ON se.dwh_row_hash = fe.dwh_row_hash
		WHERE fe.dwh_row_hash IS NULL
		)
		...
		IF @rows_source IS NULL OR @rows_source = 0 THROW 50002, 'No new records found in silver.archive_events. Load aborted.', 2;
		...
		SELECT
			nr.event_id,
			s.session_key,
			nr.[timestamp] AS event_timestamp,
			nr.event_type,
			p.product_key,
			nr.qty AS quantity,
			nr.cart_size,
			nr.payment AS payment_method,
			nr.discount_pct,
			nr.amount_usd,
			nr.dwh_batch_id,
			nr.dwh_row_hash
		...
		ON nr.product_id = p.product_id
		LEFT JOIN gold.dim_sessions s
		ON nr.[session_id] = s.user_session_id
		)
