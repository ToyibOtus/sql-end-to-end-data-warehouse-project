...
==========================================================================================================
Script Purpose:
	This script performs incremental load on the gold table [gold.fact_orders], and 
...
Usage: EXEC gold.load_gold_fact_orders;
...
==========================================================================================================
*/
CREATE OR ALTER PROCEDURE gold.load_gold_fact_orders AS
BEGIN
	...
	@table_loaded NVARCHAR(50) = 'fact_orders',
	@proc_name NVARCHAR(50) = 'load_gold_fact_orders',
	@load_type NVARCHAR(50) = 'Incremental',
	@ingest_source NVARCHAR(50) = 'silver.archive_orders',
	@start_time DATETIME,
	@end_time DATETIME,
	@load_duration INT,
	...
	BEGIN TRY
		...
		WHERE etl_layer = 'silver' and etl_table_loaded = 'archive_orders'
		ORDER BY etl_start_time DESC;

		-- Throw an error if run_id is null
		IF @run_id IS NULL THROW 50001, 'gold.fact_orders cannot load before silver.archive_orders. Load aborted.', 1;
		...
		WITH new_records AS
		(
		SELECT so.* FROM silver.archive_orders so
		LEFT JOIN gold.fact_orders fo
		ON so.dwh_row_hash = fo.dwh_row_hash
		WHERE fo.dwh_row_hash IS NULL
		)
		...
		IF @rows_source IS NULL OR @rows_source = 0 THROW 50002, 'No new records found in silver.archive_orders. Load aborted.', 2;
