...
BEGIN
	...
	BEGIN TRY
		...
		SET @layer = 'bronze';
		SET @load_type = 'Full';
		SET @proc_name = 'load_bronze_archive_events';
		SELECT @rows_source = COUNT(*) FROM staging.archive_events;
		SET @ingest_source = 'staging.archive_events';
		...
		SELECT
			...
			CONCAT_WS('|', event_id, session_id, timestamp, event_type, 
			product_id, qty, cart_size, payment, discount_pct, amount_usd) AS dwh_row_rows,
			HASHBYTES('SHA2_256', CONCAT_WS('|', event_id, session_id, timestamp, event_type, 
			product_id, qty, cart_size, payment, discount_pct, amount_usd)) AS dwh_row_hash
		FROM staging.archive_events;

		-- Map values to variables
		SET @end_time = GETDATE();
		SET @table_loaded = 'archive_events';
