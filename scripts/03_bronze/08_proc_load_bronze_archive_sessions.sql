...
BEGIN
	...
	BEGIN TRY
		...
		SET @layer = 'bronze';
		SET @load_type = 'Full';
		SET @proc_name = 'load_bronze_archive_sessions';
		SELECT @rows_source = COUNT(*) FROM staging.archive_sessions;
		SET @ingest_source = 'staging.archive_sessions';
		...
		SELECT
			...
			CONCAT_WS('|',session_id, customer_id, start_time, device, source, country) AS dwh_raw_rows,
			HASHBYTES('SHA2_256', CONCAT_WS('|',session_id, customer_id, start_time, 
			device, source, country)) AS dwh_row_hash
		FROM staging.archive_sessions;

		-- Map values to variables
		SET @end_time = GETDATE();
		SET @table_loaded = 'archive_sessions';
