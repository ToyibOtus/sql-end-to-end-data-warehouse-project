...
========================================================================================================
Script Purpose:
	This script loads data directly from staging table [staging.archive_sessions] into its
...
Usage: EXEC bronze.load_bronze_archive_sessions;
...
========================================================================================================
*/
CREATE OR ALTER  PROCEDURE bronze.load_bronze_archive_sessions AS
BEGIN
	...
	@proc_name NVARCHAR(50) = 'load_bronze_archive_sessions',
	@load_type NVARCHAR(50) = 'Full',
	@ingest_source NVARCHAR(250) = 'staging.archive_sessions',
	@start_time DATETIME,
	@end_time DATETIME,
	@load_duration INT,
	...
	BEGIN TRY
		...
		AND etl_table_loaded = 'archive_sessions' ORDER BY etl_start_time DESC;

		-- Throw an error if corresponding batch_id is NULL
		IF @batch_id IS NULL THROW 50001, 'bronze.archive_sessions cannot load before staging.archive_sessions. Load aborted.', 1;

		-- Retrieve total rows from corresponding staging table
		SELECT @rows_source = COUNT(*) FROM staging.archive_sessions;

		-- Throw an error if total rows is NULL or zero
		IF @rows_source IS NULL OR @rows_source = 0 THROW 50002, 'No records found in staging.archive_sessions. Load aborted.', 2;
		...
		TRUNCATE TABLE bronze.archive_sessions;

		-- Load data into table
		INSERT INTO bronze.archive_sessions
		(
			[session_id],
			customer_id,
			start_time,
		...
		SELECT
			...
			CONCAT_WS('|', UPPER(CAST([session_id] AS VARCHAR(50))), UPPER(CAST(customer_id AS VARCHAR(50))), 
			UPPER(CAST(start_time AS VARCHAR(50))), UPPER(CAST(device AS VARCHAR(50))), 
			UPPER(CAST(source AS VARCHAR(50))), UPPER(CAST(country AS VARCHAR(50)))) AS dwh_raw_rows,
			HASHBYTES('SHA2_256', CONCAT_WS('|', UPPER(CAST([session_id] AS VARCHAR(50))) COLLATE LATIN1_GENERAL_100_BIN2, 
			UPPER(CAST(customer_id AS VARCHAR(50))) COLLATE LATIN1_GENERAL_100_BIN2, UPPER(CAST(start_time AS VARCHAR(50))) 
			COLLATE LATIN1_GENERAL_100_BIN2, UPPER(CAST(device AS VARCHAR(50))) COLLATE LATIN1_GENERAL_100_BIN2, 
			UPPER(CAST(source AS VARCHAR(50))) COLLATE LATIN1_GENERAL_100_BIN2, UPPER(CAST(country AS VARCHAR(50))) 
			COLLATE LATIN1_GENERAL_100_BIN2)) AS dwh_row_hash
		FROM staging.archive_sessions;
		...
		SELECT @rows_loaded = COUNT(*) FROM bronze.archive_sessions;
