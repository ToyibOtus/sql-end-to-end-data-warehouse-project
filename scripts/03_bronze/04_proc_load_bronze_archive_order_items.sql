...
BEGIN
	...
	BEGIN TRY
		...
		SET @layer = 'bronze';
		SET @load_type = 'Full';
		SET @proc_name = 'load_bronze_archive_order_items';
		SELECT @rows_source = COUNT(*) FROM staging.archive_order_items;
		SET @ingest_source = 'staging.archive_order_items'
		...
		SELECT
			...
			CONCAT_WS('|', order_id, product_id, unit_price_usd, quantity, line_total_usd) AS dwh_raw_rows,
			HASHBYTES('SHA2_256', CONCAT_WS('|', order_id, product_id, unit_price_usd, 
			quantity, line_total_usd)) AS dwh_row_hash
