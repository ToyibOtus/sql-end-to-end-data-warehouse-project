...
========================================================================================================
Script Purpose:
	This script loads data directly from staging table [staging.archive_orders] into its
...
BEGIN
	...
	BEGIN TRY
		...
		WHERE etl_layer = 'staging' AND etl_table_loaded = 'archive_orders' 
		ORDER BY etl_start_time DESC;

		-- Throw an error if corresponding run_id is NULL
		IF @run_id IS NULL THROW 50001, 'bronze.archive_orders cannot load before staging.archive_orders. Load aborted.', 1;
		...
		-- Throw an error if total rows is NULL
		...
		INSERT INTO bronze.archive_orders
		(
			order_id,
			customer_id,
			order_time,
