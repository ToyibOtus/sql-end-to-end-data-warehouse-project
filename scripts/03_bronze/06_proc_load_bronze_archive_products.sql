...
BEGIN
	...
	BEGIN TRY
		...
		WHERE etl_layer = 'staging' AND etl_table_loaded = 'archive_products' 
		ORDER BY etl_start_time DESC;

		-- Throw an error if corresponding run_id is NULL
		IF @run_id IS NULL THROW 50001, 'bronze.archive_products cannot load before staging.archive_products. Load aborted.', 1;
